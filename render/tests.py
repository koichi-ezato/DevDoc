...
class RenderViewTest(TestCase):
    ...
    def test_index(self):
        client = Client()
        client.login(username='596560', password='koichi-ezato')

        response = client.get(path='/render/')
        self.assertRedirects(response=response, expected_url='/settings/list/')

        data = {'guid': '8b1dbb7b-4dd2-49be-b7e7-e323e9399fd9'}
        response = client.get(path='/render/', data=data)
        self.assertEqual(client.session['note_guid'], '8b1dbb7b-4dd2-49be-b7e7-e323e9399fd9')
